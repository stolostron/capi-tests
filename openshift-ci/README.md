...
4. The **e2e workflow** chains: IPI Azure cluster provisioning → e2e tests (with controller deployment) → deprovisioning
...
```
IPI Azure Provisioning → Run E2E Tests (deploys controllers) → Deprovision
       (~40 min)                    (~90 min)                      (~15 min)
```
