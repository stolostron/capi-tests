...
// TestDeployment_ApplyIndividualFiles tests applying each provider-specific YAML file individually.
// Creates a subtest for each file (ARO: credentials.yaml, aro.yaml; ROSA: secrets.yaml, is.yaml, rosa.yaml)
func TestDeployment_ApplyIndividualFiles(t *testing.T) {
	config := NewTestConfig()
	...
	// Verify cluster is healthy once before applying any resources
	if err := WaitForClusterHealthy(t, context, DefaultHealthCheckTimeout); err != nil {
		t.Fatalf("Cluster health check failed: %v", err)
	}

	// Apply each file as a separate subtest
	for _, file := range expectedFiles {
