...
func TestDeployment_ApplyResources(t *testing.T) {
	...
	applied, err := applyFilesInOrder(expectedFiles, func(file string) error {
		filePath := filepath.Join(outputDir, file)
		if !FileExists(filePath) {
			return fmt.Errorf("cannot apply missing file: %s", file)
		}

		PrintToTTY("Applying resource file: %s...\n", file)
		t.Logf("Applying resource file: %s", file)
		return ApplyWithRetry(t, context, filePath, DefaultApplyMaxRetries)
	})
	if err != nil {
		PrintToTTY("❌ Partial resource application: %v\n", err)
		t.Fatalf("Partial resource application failed: %v; successfully applied files: %v", err, applied)
	}
...
func TestDeployment_ApplyClusterYAMLs(t *testing.T) {
	...
	// Apply each file in order and stop immediately after the first failure.
	applied, err := applyFilesInOrder(expectedFiles, func(file string) error {
		filePath := filepath.Join(outputDir, file)
		if !FileExists(filePath) {
			return fmt.Errorf("%s not found at %s", file, filePath)
		}

		PrintToTTY("Applying %s...\n", file)
