...
func DumpNotReadyResourceDiagnostics(t *testing.T, context, namespace string, notReady []AROClusterResourceStatus) {
	...
	PrintToTTY("\n=== DIAGNOSTICS: Not-ready infrastructure resources ===\n")
	t.Logf("Dumping diagnostics for %d not-ready resources", len(notReady))

	for _, r := range notReady {
		resourceType := strings.ToLower(r.Resource.Kind) + "s"
		if r.Resource.Group != "" {
		...
		if err == nil && strings.TrimSpace(condOutput) != "" {
			...
			PrintToTTY("Could not fetch conditions: %v\n", err)
		}
...
// WriteDiagnosticsFile writes diagnostic information to a file in the results directory.
// This ensures diagnostics survive process termination (e.g., Prow entrypoint timeout).
func WriteDiagnosticsFile(t *testing.T, filename string, content string) {
	t.Helper()
	resultsDir := GetResultsDir()
	filePath := filepath.Join(resultsDir, filename)
	if err := os.WriteFile(filePath, []byte(content), 0644); err != nil { // #nosec G306 -- diagnostics file, not sensitive
		t.Logf("Warning: failed to write diagnostics file %s: %v", filePath, err)
	} else {
		t.Logf("Diagnostics written to %s", filePath)
	}
}
