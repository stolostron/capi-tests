...
| K8S | `USE_K8S=true` | External K8s cluster | Deployed by tests | `multicluster-engine` | Generic Kubernetes | v1 |
...
**Description:**
Uses an external Kubernetes cluster with pre-installed CAPI/CAPZ/ASO controllers. The test suite does not deploy controllers - it validates they exist and proceeds with workload cluster deployment.

**Prerequisites:**
- Access to external Kubernetes cluster
...
- Deployed to `multicluster-engine` namespace
...
- CAPI, CAPZ, ASO controllers pre-installed
...
Do you have MCE cluster access?
  ...
  +-- No
        |
        v
      Do you have external K8s cluster?
        |
...
| OCP | `multicluster-engine` | `multicluster-engine` |
