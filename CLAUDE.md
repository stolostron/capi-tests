...
### Test Behavior
- `CLUSTER_DEPLOYMENT_TIMEOUT` - How long the in-code polling loop waits for the workload cluster to become ready (default: `60m`, format: Go duration like `1h`, `45m`). The Makefile's `GO_STEP_DEPLOY_CRS_TIMEOUT` is auto-computed as this value + 15 minutes headroom.
- `CLUSTER_DELETION_TIMEOUT` - How long the in-code polling loop waits for the workload cluster to be deleted (default: `60m`, format: Go duration). The Makefile's `GO_STEP_DELETION_TIMEOUT` is auto-computed as this value + 15 minutes headroom.
